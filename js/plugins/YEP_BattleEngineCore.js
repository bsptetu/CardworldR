...
BattleManager.actionAniWait = function(frames) {
    ...
    this._logWindow._waitCount = 1;parseInt(frames);
    return false;
};
...
//this.startMove(Yanfly.Param.BECStepDist, 15, 8);
};
...
Game_Battler.prototype.performActionStart = function(action) {
    if (!action.isGuard() && action.isAttack()) {
        this.setActionState('acting');
        this.spriteStepForward();
    }
    if (!action.isGuard() && action.isPhysical()) {
        this.setActionState('acting');
        this.spriteStepForward();
    }
};
